...
MyMessage msg_MQ(CHILD_ID_MQ, V_LEVEL);
MyMessage msg_DS(CHILD_ID_DS, V_TEMP);
void setup()
{
...
void presentation()
{
  ...
  sendSketchInfo("Air Control Sensor", "1.0");

  // Register all sensors to gateway (they will be created as child devices)
  present(CHILD_ID_MQ, S_AIR_QUALITY, "Gas Control Sensor");
  present(CHILD_ID_DS, S_TEMP, "Temp control sensor");
...
//  send(msg_DS.setSensor(2).set(temperature,2));
  send(msg_DS.set(temperature, 2));
  // Save new temperatures for next compare
  lastTemperature[0] = temperature;
  
  if (valMQ != lastMQ) {
//    send(msg_MQ.setSensor(3).set((int16_t)ceil(valMQ),3));
    send(msg_MQ.set((int16_t)ceil(valMQ), 3));
    lastMQ = ceil(valMQ);
  }
